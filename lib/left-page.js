...
function setBackgroundColor(pugCode, tag, color) {
	return pugCode.replace(tag, `bgcolor=${color}`)
}

function setBackgroundImage(pugCode, tag, imageName) {
	return pugCode.replace(tag, `background=/images/${imageName}`)
}

function applyTheme(pugCode, theme) {
	// Body background
	if (theme.backgroundColor) {
		pugCode = setBackgroundColor(pugCode, 'BODY_BACKGROUND_TAG', theme.backgroundColor);
	} else {
		pugCode = setBackgroundImage(pugCode, 'BODY_BACKGROUND_TAG', theme.backgroundImage);
	}
	// Border color
	pugCode = pugCode.replace('FRAME_BORDER_TAG', `'${theme.frameBorder}'`)
}
